---
title: "Monocle analysis for SPIG1 and HB9"
...
```{r}
setwd("/Users/al-khindi/Documents/Johns Hopkins University/Graduate School/Kolodkin Lab/Thesis/RNAseq project/2nd RNAseq batch/SPIG1-GFP and HB9-GFP experiment/cuffnorm_output/")
...
#remove all HB9-GFP cells
ordered_fpkms=ordered_fpkms[,-grep("^H",colnames(ordered_fpkms))]
#ordered_isoform_fpkms=ordered_isoform_fpkms[,-grep("^H",colnames(ordered_isoform_fpkms))]
ordered_sample_ann=ordered_sample_ann[-grep("^H",rownames(ordered_sample_ann)),]
ordered_master_cell_sheet=ordered_master_cell_sheet[-grep("^H",rownames(ordered_master_cell_sheet)),]
...
#remove all Down SPIG1- cells
ordered_fpkms=ordered_fpkms[,-grep("^D",colnames(ordered_fpkms))]
#ordered_isoform_fpkms=ordered_isoform_fpkms[,-grep("^D",colnames(ordered_isoform_fpkms))]
ordered_sample_ann=ordered_sample_ann[-grep("^D",rownames(ordered_sample_ann)),]
ordered_master_cell_sheet=ordered_master_cell_sheet[-grep("^D",rownames(ordered_master_cell_sheet)),]
...
#only include cells in Cluster 2 + 3 (Up + large group Down oDSGCs)
desiredcells=read.xlsx("../Results/Monocle/SPIG1 Up vs Down vs Pcdh9/Excel sheets/Cells belonging to each cluster.xlsx")
desiredcelllist=desiredcells[desiredcells$cluster_id==c(2,3),1]
ordered_fpkms=ordered_fpkms[, colnames(ordered_fpkms) %in% c(desiredcelllist)]
ordered_sample_ann=ordered_sample_ann[rownames(ordered_sample_ann) %in% c(desiredcelllist), ]
ordered_master_cell_sheet=ordered_master_cell_sheet[rownames(ordered_master_cell_sheet) %in% c(desiredcelllist), ]
...
```{r}
#Semi-supervised clustering, round 1
...
#in command below, num_clusters should be set to "number of expected clusters + 1" (i.e., if you see 3 clusters, num_clusters should be 4)
...
```{r}
#Semi-supervised clustering, round 2 (from round 1, I determined that there are 3 clusters)

#Use Nr2f2- (Cluster 1), Fibcd1+ (Cluster 2), and Fstl4+ (Cluster 3) as marker genes

Fstl4_id <- row.names(subset(fData(dat), gene_short_name == "Fstl4"))
Fibcd1_id <- row.names(subset(fData(dat), gene_short_name == "Fibcd1"))
Nr2f2_id <- row.names(subset(fData(dat), gene_short_name == "Nr2f2"))

cth <- newCellTypeHierarchy()
cth <- addCellType(cth, "Clus3", classify_func =
    function(x) { x[Fstl4_id,] >= 1 })
cth <- addCellType(cth, "Clus2", classify_func =
    function(x) { x[Fibcd1_id,] >= 1 })
cth <- addCellType(cth, "Clus1", classify_func =
    function(x) { x[Nr2f2_id,] <= 1 })
...
numCellThreshold<-4 # Maybe lower this
expressed_genes<-row.names(subset(fData(dat),num_cells_expressed >= numCellThreshold))
...
gl=read.xlsx("~/Documents/Johns Hopkins University/Graduate School/Kolodkin Lab/Thesis/RNAseq project/2nd RNAseq batch/SPIG1-GFP and HB9-GFP experiment/Results/Monocle/SPIG1 Up vs Down vs Pcdh9/Excel sheets/DE - cluster3 vs all.xlsx")
to_test=gl$gene_short_name
exp_lvl <- row.names(subset(fData(dat), gene_short_name %in% to_test))
exp_list <- dat[exp_lvl,]
exp_list=as.data.frame(exp_list)
x=subset(gene_ann, gene_short_name %in% to_test)
#colnames(exp_list)[1:nrow(x)]=as.character(x$gene_short_name)
gt=aggregate(exp_list[1:nrow(x)],by=list(exp_list$cluster3), FUN=mean)
...
```{r}
#Pseudotime - Supervised ordering using differentialGeneTest between P5 and P8
...
plot_pseudotime_heatmap(dat[sig_gene_names,],
num_clusters = 2,
cores = 1,
show_rownames = T,
use_gene_short_name = T)

#Branched Expression Analysis Modeling (BEAM) for HB9-GFP cells
BEAM_res <- BEAM(dat, branch_point = 1, cores = 8)
#above step is done within 1 hr 6 min
BEAM_res <- BEAM_res[order(BEAM_res$qval),]
BEAM_res <- BEAM_res[,c("gene_short_name", "pval", "qval")]

sig_genes1 <- subset(BEAM_res, qval < 0.05)

write.xlsx(sig_genes1,"branch_1_genes.xlsx")

plot_genes_branched_heatmap(dat[row.names(subset(BEAM_res,
                                          qval < 1e-4)),],
                                          branch_point = 1,
                                          num_clusters = 4,
                                          cores = 8,
...
branch1_genes <- row.names(subset(fData(dat),
          gene_short_name %in% c("Gm3002", "Gm6157", "Marcksl1", "Nnat", "Crip2")))
plot_genes_branched_pseudotime(dat[branch1_genes,],
                       branch_point = 1,
                       color_by = "age",
                       ncol = 1)
...
```{r}
#make volcano plot of cluster 3 vs. all

library(openxlsx)
library(ggplot2)
setwd("/Users/al-khindi/Documents/Johns Hopkins University/Graduate School/Kolodkin Lab/Thesis/RNAseq project/2nd RNAseq batch/SPIG1-GFP and HB9-GFP experiment/Results/Monocle/SPIG1 Up vs Down vs Pcdh9/Excel sheets/")
vp=read.xlsx("cluster 3 vs all - volcano plot.xlsx")

#volcano plot without labels
ggplot(data=vp, aes(x=fc_enrich, y=neg_log_qval, group=1))+
  geom_point()+
  theme_bw()+
...
#volcano plot with labels above a threshold
ggplot(data=vp, aes(x=fc_enrich, y=neg_log_qval, group=1))+
  geom_point()+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  ...
  xlab("Fold change")+
  ylab(bquote('-log'[10]*' q-value'))+
  geom_text(data=subset(vp, fc_enrich > 25 | fc_enrich < -25 | neg_log_qval > 40), aes(fc_enrich, neg_log_qval, label=gene_short_name))

#volcano plot with log2 FC + labels above a threshold
ggplot(data=vp, aes(x=log2_fc, y=neg_log_qval, group=1))+
  geom_point()+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  ...
  ylab(bquote('-log'[10]*' q-value'))+
  geom_text(data=subset(vp, log2_fc > 3.8 | fc_enrich < -4.5 | neg_log_qval > 20), aes(log2_fc, neg_log_qval, label=gene_short_name), check_overlap = T, vjust = 0, nudge_y = 1)

#volcano plot with log2 FC + label transmembrane proteins
ggplot(data=vp, aes(x=log2_fc, y=neg_log_qval, group=1))+
  geom_point()+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  ...
  xlab(bquote('log'[2]*' fold change'))+
  ylab(bquote('-log'[10]*' q-value'))+
  geom_text(data=subset(vp, Transcription.factor==TRUE), aes(log2_fc, neg_log_qval, label=gene_short_name), check_overlap = T, vjust = 0, nudge_y = 1, colour="blue")+
  geom_point(data=subset(vp, Transcription.factor==TRUE), colour="blue")
...
ggplot(data=vp, aes(x=log2_fc, y=neg_log_qval, group=1))+
    ...
    xlab(bquote('log'[2]*' fold change'))+
    ylab(bquote('-log'[10]*' q-value'))+
    geom_text(data=subset(vp, gene_short_name=="Tbx5"), aes(log2_fc, neg_log_qval, label=gene_short_name), check_overlap = T, vjust = 0, nudge_y = 1, colour="blue", size = 7)+
    geom_point(data=subset(vp, gene_short_name=="Tbx5"), colour="blue")+
    theme(text = element_text(size=30))+
  
  geom_text(data=subset(vp, gene_short_name=="Ptprk"), aes(log2_fc, neg_log_qval, label=gene_short_name), check_overlap = T, vjust = 0, nudge_y = 1, colour="blue", size = 7)+
    geom_point(data=subset(vp, gene_short_name=="Ptprk"), colour="blue")+
  
    geom_text(data=subset(vp, gene_short_name=="Fstl4"), aes(log2_fc, neg_log_qval, label=gene_short_name), check_overlap = T, vjust = 0, nudge_y = 1, colour="blue", size = 7)+
    geom_point(data=subset(vp, gene_short_name=="Fstl4"), colour="blue")+
  
  geom_text(data=subset(vp, gene_short_name=="Fibcd1"), aes(log2_fc, neg_log_qval, label=gene_short_name), check_overlap = T, vjust = 0, nudge_y = 1, colour="blue", size = 7)+
    geom_point(data=subset(vp, gene_short_name=="Fibcd1"), colour="blue")+
  
  geom_text(data=subset(vp, gene_short_name=="Efnb2"), aes(log2_fc, neg_log_qval, label=gene_short_name), check_overlap = T, vjust = 0, nudge_y = 1, colour="blue", size = 7)+
    geom_point(data=subset(vp, gene_short_name=="Efnb2"), colour="blue")+

   geom_text(data=subset(vp, gene_short_name=="Ptprm"), aes(log2_fc, neg_log_qval, label=gene_short_name), check_overlap = T, vjust = 0, nudge_y = 1, colour="blue", size = 7)+
    geom_point(data=subset(vp, gene_short_name=="Ptprm"), colour="blue")+
  
   geom_text(data=subset(vp, gene_short_name=="Ptprn"), aes(log2_fc, neg_log_qval, label=gene_short_name), check_overlap = T, vjust = 0, nudge_y = 1, colour="blue", size = 7)+
    geom_point(data=subset(vp, gene_short_name=="Ptprn"), colour="blue")+
  
    geom_text(data=subset(vp, gene_short_name=="Ptpre"), aes(log2_fc, neg_log_qval, label=gene_short_name), check_overlap = T, vjust = 0, nudge_y = 1, colour="blue", size = 7)+
    geom_point(data=subset(vp, gene_short_name=="Ptpre"), colour="blue")+

     geom_text(data=subset(vp, gene_short_name=="Ptpro"), aes(log2_fc, neg_log_qval, label=gene_short_name), check_overlap = T, vjust = 0, nudge_y = 1, colour="blue", size = 7)+
    geom_point(data=subset(vp, gene_short_name=="Ptpro"), colour="blue")
...
ggplot(data=vp, aes(x=log2_fc, y=neg_log_qval, group=1))+
    ...
    theme(text = element_text(size=30))+
    geom_point(data=subset(vp, gene_short_name=="Tbx5"), colour="blue", size=2)+
    geom_point(data=subset(vp, gene_short_name=="Ptprk"), colour="blue", size=2)+
    geom_point(data=subset(vp, gene_short_name=="Fstl4"), colour="blue", size=2)+
    geom_point(data=subset(vp, gene_short_name=="Fibcd1"), colour="blue", size=2)+
    geom_point(data=subset(vp, gene_short_name=="Sfrp1"), colour="blue", size=2)
